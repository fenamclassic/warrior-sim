...
SIM.UI = {
  ...
  events: function() {
    ...
    view.main.find('nav li p').click(function(e) {
      ...
      else if (type == 'custom') view.loadCustom();
      else view.loadGear(type);
    });
    ...
    view.tcontainer.on('click', 'table.gear td:not(.ppm)', function(e) {
      ...
      if (table.hasClass('editmode')) {
        if (tr.hasClass('hidden')) view.rowShowItem(tr);
        else view.rowHideItem(tr);
        return;
      }
    ...
    view.tcontainer.on('click', 'table.enchant td:not(.ppm)', function(e) {
      ...
      if (table.hasClass('editmode')) {
        if (tr.hasClass('hidden')) view.rowShowEnchant(tr);
        else view.rowHideEnchant(tr);
        return;
      }
  ...
  enableEditMode: function() {
    ...
    else if (type == 'custom') view.loadCustom(true);
    else view.loadGear(type, true);
  },
  ...
  disableEditMode: function() {
    ...
    else if (type == 'custom') view.loadCustom(false);
    else view.loadGear(type, false);
  },

  simulateDPS: function(row) {
    let view = this;
    let dps = view.sidebar.find('#dps');
    let statsmin = view.sidebar.find('#statsmin');
    let statsmax = view.sidebar.find('#statsmax');
    let tps = view.sidebar.find('#tps');
    let tpsstatsmin = view.sidebar.find('#tpsstatsmin');
    let tpsstatsmax = view.sidebar.find('#tpsstatsmax');
    let dtps = view.sidebar.find('#dtps');
    let dtpsstatsmin = view.sidebar.find('#dtpsstatsmin');
    let dtpsstatsmax = view.sidebar.find('#dtpsstatsmax');
    ...
    var target = new Target(player, parseInt($('input[name="targetlevel"]').val()), parseInt($('input[name="basedamage"]').val()), parseFloat($('input[name="baseattacktime"]').val()));
    var sim = new Simulation(
      ...
      () => {
        ...
        tps.text((sim.totalthreat / sim.totalduration).toFixed(2));
        dtps.text((sim.totaldmgtaken / sim.totalduration).toFixed(2));
        time.text((sim.endtime - sim.starttime) / 1000);
        statsmin.html(sim.mindps.toFixed(2) + ' min');
        statsmax.html(sim.maxdps.toFixed(2) + ' max');
        tpsstatsmin.html(sim.mintps.toFixed(2) + ' min');
        tpsstatsmax.html(sim.maxtps.toFixed(2) + ' max');
        dtpsstatsmin.html(sim.mindtps.toFixed(2) + ' min');
        dtpsstatsmax.html(sim.maxdtps.toFixed(2) + ' max');
        btn.css('background', '');
  ...
  simulateRow: function(tr) {
    var view = this;
    var dps = tr.find('.td-dps');
    var tps = tr.find('.td-tps');
    var dtps = tr.find('.td-dtps');
    ...
    var basedtps = parseFloat(view.sidebar.find('#dtps').text());
    ...
    var sim = new Simulation(
      ...
      () => {
        ...
        let calctps = sim.totalthreat / sim.totalduration;
        ...
        let spandtps = $('<span></span>');
        let calcdtps = sim.totaldmgtaken / sim.totalduration;
        let diffdtps = calcdtps - basedtps;
        spandtps.text(diffdtps.toFixed(2));
        if (diffdtps >= 0) spandtps.addClass('p');
        else spandtps.addClass('n');
        dtps.text(calcdtps.toFixed(2)).append(spandtps);
        ...
        let perc = parseInt(
          (((rowsdone + 1) * sim.iterations) / (sim.iterations * rows)) * 100
        );
      ...
      iteration => {
        // Update
        let perc = parseInt(
          ((rowsdone * sim.iterations + iteration) / (sim.iterations * rows)) *
            100
        );
        btn.css(
          'background',
  ...
  rowDisableItem: function(tr) {
    ...
    for (let i = 0; i < gear[type].length; i++) {
      if (gear[type][i].id == tr.data('id')) gear[type][i].selected = false;
    }
  },
  ...
  rowEnableItem: function(tr) {
    ...
    tr.addClass('active');
    for (let i = 0; i < gear[type].length; i++) {
      if (gear[type][i].id == tr.data('id')) gear[type][i].selected = true;
    ...
    if (type == 'twohand') {
      ...
      for (let i = 0; i < gear.shield.length; i++)
        gear.shield[i].selected = false;
      ...
      for (let i = 0; i < enchant.shield.length; i++)
        enchant.shield[i].selected = false;
    }
  ...
  rowShowItem: function(tr) {
    ...
    for (let i = 0; i < gear[type].length; i++) {
      if (gear[type][i].id == tr.data('id')) gear[type][i].hidden = false;
    }
  },
  ...
  rowShowEnchant: function(tr) {
    ...
    tr.find('.hide').html(eyesvg);
    for (let i = 0; i < enchant[type].length; i++) {
      if (enchant[type][i].id == tr.data('id')) enchant[type][i].hidden = false;
    }
  },
  ...
  updateSidebar: function() {
    ...
    view.sidebar
      .find('#skill')
      .html(
        player.stats['skill_' + player.mh.type] +
          ' <small>MH</small>' +
          (player.oh
            ? space +
              player.stats['skill_' + player.oh.type] +
              ' <small>OH</small>'
            : '')
      );
    ...
    view.sidebar
      .find('#miss')
      .html(
        Math.max(player.mh.miss, 0).toFixed(2) +
          '% <small>1H</small>' +
          (player.oh
            ? space +
              Math.max(player.mh.dwmiss, 0).toFixed(2) +
              '% <small>DW</small>'
            : '')
      );
    ...
    view.sidebar
      ...
      .html(
        mhcrit.toFixed(2) +
          '% <small>MH</small>' +
          (player.oh ? space + ohcrit.toFixed(2) + '% <small>OH</small>' : '')
      );
    let mhcap =
      100 - player.mh.dwmiss - player.mh.dodge - player.mh.glanceChance;
    let ohcap = player.oh
      ? 100 - player.oh.dwmiss - player.oh.dodge - player.oh.glanceChance
      : 0;
    view.sidebar
      .find('#critcap')
      .html(
        mhcap.toFixed(2) +
          '% <small>MH</small>' +
          (player.oh ? space + ohcap.toFixed(2) + '% <small>OH</small>' : '')
      );
    let mhdmg = player.stats.dmgmod * player.mh.modifier * 100;
    let ohdmg =
      player.stats.dmgmod * (player.oh ? player.oh.modifier * 100 : 0);
    view.sidebar
      .find('#dmgmod')
      .html(
        mhdmg.toFixed(2) +
          '% <small>MH</small>' +
          (player.oh ? space + ohdmg.toFixed(2) + '% <small>OH</small>' : '')
      );
    view.sidebar
      .find('#haste')
      .html((player.stats.haste * 100).toFixed(2) + '%');
    view.sidebar.find('#race').text(localStorage.race);

    view.sidebar.find('#sta').text(player.stats.sta);
    view.sidebar.find('#armor').text(player.stats.ac);
    view.sidebar.find('#defense').text(player.stats.defense);
    view.sidebar.find('#dodge').text(player.stats.dodge.toFixed(2) + '%');
    view.sidebar.find('#parry').text(player.stats.parry.toFixed(2) + '%');
    view.sidebar.find('#block').text(player.stats.block.toFixed(2) + '%');
    view.sidebar.find('#blockvalue').text(player.stats.blockvalue);
  },
  ...
  updateSession: function() {
    ...
    localStorage.simulations = view.fight
      .find('input[name="simulations"]')
      .val();
    localStorage.timesecsmin = view.fight
      .find('input[name="timesecsmin"]')
      .val();
    localStorage.timesecsmax = view.fight
      .find('input[name="timesecsmax"]')
      .val();
    localStorage.startrage = view.fight.find('input[name="startrage"]').val();
    localStorage.targetlevel = view.fight
      .find('input[name="targetlevel"]')
      .val();
    localStorage.targetarmor = view.fight
      .find('input[name="targetarmor"]')
      .val();
    localStorage.targetresistance = view.fight
      .find('input[name="targetresistance"]')
      .val();
    localStorage.adjacent = view.fight.find('input[name="adjacent"]').val();
    localStorage.adjacentlevel = view.fight
      .find('input[name="adjacentlevel"]')
      .val();
    ...
    for (let tree of talents) {
      let arr = [];
      for (let talent of tree.t) arr.push(talent.c);
  ...
  loadSession: function() {
    ...
    view.sidebar
      .find('.bg')
      .attr('data-race', view.fight.find('select[name="race"]').val());

    let _buffs = !localStorage.buffs
      ? session.default.buffs
      : JSON.parse(localStorage.buffs);
    let _rotation = !localStorage.rotation
      ? session.default.rotation
      : JSON.parse(localStorage.rotation);
    let _sources = !localStorage.sources
      ? session.default.sources
      : JSON.parse(localStorage.sources);
    let _phases = !localStorage.phases
      ? session.default.phases
      : JSON.parse(localStorage.phases);
    let _talents = !localStorage.talents
      ? session.default.talents
      : JSON.parse(localStorage.talents);
    let _gear = !localStorage.gear
      ? session.default.gear
      : JSON.parse(localStorage.gear);
    let _enchant = !localStorage.enchant
      ? session.default.enchant
      : JSON.parse(localStorage.enchant);
    ...
    for (let i of _buffs) for (let j of buffs) if (i == j.id) j.active = true;

    for (let i of _rotation)
      for (let j of spells)
        if (i.id == j.id) for (let prop in i) j[prop] = i[prop];
  ...
  filterGear: function() {
    ...
    if (type == 'mainhand' || type == 'offhand') view.loadWeapons(type);
    else if (type == 'custom') view.loadCustom();
    else view.loadGear(type);
  },
  ...
  loadWeapons: function(type, editmode) {
    ...
    let table = `<table class="gear ${
      editmode ? 'editmode' : ''
    }" data-type="${type}" data-max="1">
                        <thead>
                            <tr>
                                ${editmode ? '<th></th>' : ''}
                                <th>Name</th>
    ...
    for (let item of gear[type]) {
      if (filter && filter != 'All') {
        if (filter == 'Mace & Sword') {
          ...
          if (
            ...
            continue;
        } else if (item.type != filter) continue;
      }
      ...
      table += `<tr data-id="${item.id}" data-name="${item.name}" class="${
        item.selected ? 'active' : ''
      } ${item.hidden ? 'hidden' : ''}">
                        ${
                          editmode
                            ? '<td class="hide">' +
                              (item.hidden ? eyesvghidden : eyesvg) +
                              '</td>'
                            : ''
                        }
                        <td><a href="https://classic.wowhead.com/item=${tooltip}${rand}"></a>${
        item.name
      }</td>
                        <td>${item.source}</td>
                        <td>${item.sta || ''}</td>
                        ...
                        <td>${item.skill || ''}</td>
                        <td>${item.type || ''}</td>
                        <td class="ppm"><p contenteditable="true">${item.ppm ||
                          ''}</p></td>
                        <td class="td-dps">${item.dps || ''}</td>
                        <td class="td-tps">${item.tps || ''}</td>
                        <td class="td-dtps">${item.dtps || ''}</td>
                    </tr>`;
    }
  ...
  loadGear: function(type, editmode) {
    ...
    for (let item of gear[type]) {
      ...
      table += `<tr data-id="${item.id}" class="${
        item.selected ? 'active' : ''
      } ${item.hidden ? 'hidden' : ''}">
                        ${
                          editmode
  ...
  loadEnchants: function(type, editmode) {
    ...
    for (let item of enchant[type]) {
      if (
        item.phase &&
        !view.filter
          .find('.phases [data-id="' + item.phase + '"]')
          .hasClass('active')
      )
        continue;
      ...
      table += `<tr data-id="${item.id}" data-temp="${item.temp ||
        false}" class="${item.selected ? 'active' : ''} ${
        item.hidden ? 'hidden' : ''
      }">
                        ${
                          editmode
                        ...
                        <td><a href="https://classic.wowhead.com/${
                          item.spellid ? 'spell' : 'item'
                        }=${item.id}"></a>${item.name}</td>
                        <td>${item.str || ''}</td>
                        <td>${item.agi || ''}</td>
                        <td>${item.sta || ''}</td>
                        <td>${item.ap || ''}</td>
